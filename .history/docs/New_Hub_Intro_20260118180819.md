...
为了提升文档的可读性和专业感，我为您美化了 **Jupyter AI 智能对话** 部分。采用更清晰的列表结构、代码块样式以及提示框，让用户一眼就能看清常用指令。

您可以将这部分直接替换到您的 Markdown 文档中：

---

### 💬 智能对话 (Chat)

遇到不懂的代码或数学原理？直接在左侧边栏的 **Jupyternaut** 图标处提问。它支持上下文感知，能极大地提升您的开发效率。

#### 🛠️ 常用快捷指令

在对话框中输入 `/` 即可快速调用以下功能：

* **`/fix`** — 自动分析并尝试修复 Notebook 中选中的错误单元格。
* **`/learn`** — 让 AI 学习您本地的项目文件，使其回答更贴合您的业务逻辑。
* **`/generate`** — 根据您的文字描述，直接生成一个完整的 Jupyter Notebook 文件。
* **`/ask`** — 针对已学习（Learned）的数据进行深度提问。
* **`/export`** — 将当前的对话记录导出为 Markdown 文件。

#### 📎 引用本地上下文

* **`@file`** — 在提问时输入 `@` 并选择文件，可将指定文件的内容作为背景知识提供给 AI。
...
针对天文数据处理等高负载任务，我们新增了实时资源监控面板，方便您随时查看 **GPU 显存占用**及系统负载。

* **入口**: 点击左侧边栏的 **Dashboard** 图标。
* **功能**: 实时显示 GPU 显存、计算核负载及 CPU/内存状态，确保深度学习任务平稳运行。
